...
<div style="background:#ffffff;border-radius:20px;overflow:hidden;min-width:0;">
    <form method="GET" action="{{ route('item-catalog') }}" style="padding:20px 24px 16px 24px;display:flex;align-items:center;gap:12px;flex-wrap:nowrap;min-width:0;">
        ...
        <div id="itemsFilters" style="display:flex;align-items:center;gap:12px;flex:1;min-width:0;">
            ...
            @if(request()->anyFilled(['search', 'category', 'warehouse', 'status']))
                <a href="{{ route('item-catalog') }}" style="background:transparent;color:#64748b;border:1px solid #cbd5e1;border-radius:20px;padding:8px 16px;font-size:13px;font-family:'Inter',sans-serif;text-decoration:none;display:inline-flex;align-items:center;gap:4px;white-space:nowrap;flex-shrink:0;">
            ...
            <table class="stock-table" style="width:100%;table-layout:fixed;border-collapse:collapse;min-width:820px;">
                ...
                <tbody>
                    ...
                    <tr style="background:{{ $loop->even ? '#F4F6FA' : '#ffffff' }}; border-top:1px solid #E2E8F0; cursor:pointer;" onclick="toggleExpand({{ $loop->index }})">
                        ...
                        <td style="text-align:center;padding:12px 4px;">
                            <form method="POST" action="{{ route('item-catalog.destroy', $item['id']) }}" onsubmit="return confirm('Delete this item permanently?')" style="display:inline;">
                                @csrf @method('DELETE')
            ...
            <table class="stock-table" style="width:100%;table-layout:auto;border-collapse:collapse;">
                ...
                <tbody>
                    ...
                    <tr style="border-bottom:1px solid #e2e8f0;background:{{ $loop->even ? '#F4F6FA' : '#fff' }};">
                        ...
                        <td style="text-align:center;padding:12px 6px;">
                            <form method="POST" action="{{ route('item-catalog.packing.destroy', $pm->id) }}" onsubmit="return confirm('Delete this packing material?')" style="display:inline;">
                                @csrf @method('DELETE')
                                <button type="submit" style="background:none;border:none;cursor:pointer;color:#dc2626;font-size:13px;padding:4px 8px;">🗑</button>
                            </form>
                        </td>
